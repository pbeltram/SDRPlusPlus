...
SDRPP_MOD_INFO
{
  /* Name:            */ "papi_sdr_source",
  /* Description:     */ "PapiSDR source module for SDR++",
  /* Author:          */ "",
  /* Version:         */ 0, 1, 0,
  /* Max instances    */ 1
};
...
//----------------------------------------------------------------------------//
class PapiSDRSourceModule : public ModuleManager::Instance {

public:
//----------------------------------------------------------------------------//
PapiSDRSourceModule(std::string name)
{
  this->name = name;
  ...
  sigpath::sourceManager.registerSource("PapiSDR", &handler);
}
//----------------------------------------------------------------------------//
...
{
  stop(this);
  sigpath::sourceManager.unregisterSource("PapiSDR");
}
//----------------------------------------------------------------------------//
...
static void menuSelected(void* ctx)
{
  ...
  spdlog::info("PapiSDRSourceModule '{0}': Menu Select!", _this->name);
}
//----------------------------------------------------------------------------//
...
{
  PapiSDRSourceModule* _this = (PapiSDRSourceModule*)ctx;
  spdlog::info("PapiSDRSourceModule '{0}': Menu Deselect!", _this->name);
}
//----------------------------------------------------------------------------//
...
static void start(void* ctx)
{
  ...
  if (_this->ctx == NULL) {
    spdlog::error("Could not open papisdr");
    return;
  }

  _this->phy = iio_context_find_device(_this->ctx, "ad9361-phy");
  if (_this->phy == NULL) {
    spdlog::error("Could not connect to papisdr phy");
  ...
  if (_this->dev == NULL) {
    spdlog::error("Could not connect to papisdr dev");
    iio_context_destroy(_this->ctx);
    return;
  }
  ...
  spdlog::info("PapiSDRSourceModule '{0}': Start!", _this->name);
}
//----------------------------------------------------------------------------//
...
static void stop(void* ctx)
{
  ...
  spdlog::info("PapiSDRSourceModule '{0}': Stop!", _this->name);
}
//----------------------------------------------------------------------------//
...
static void tune(double freq, void* ctx)
{
  ...
  spdlog::info("PapiSDRSourceModule '{0}': Tune: {1}!", _this->name, freq);
}
//----------------------------------------------------------------------------//
...
static void menuHandler(void* ctx)
{
  ...
  ImGui::SetNextItemWidth(menuWidth - ImGui::GetCursorPosX());
  if (_this->gainMode == 0) { style::beginDisabled(); }
  ...
  if (_this->gainMode == 0) { style::endDisabled(); }
}
//----------------------------------------------------------------------------//
...
{
  json defConf;
  defConf["IP"] = "169.254.50.80";
  defConf["sampleRate"] = 4000000.0f;
  defConf["gainMode"] = 0;
  defConf["gain"] = 0.0f;
  config.setPath(options::opts.root + "/papi_sdr_source_config.json");
  config.load(defConf);
...
{
  return new PapiSDRSourceModule(name);
}
//----------------------------------------------------------------------------//

//----------------------------------------------------------------------------//
MOD_EXPORT void _DELETE_INSTANCE_(ModuleManager::Instance* instance) {
  delete (PapiSDRSourceModule*)instance;
}
//----------------------------------------------------------------------------//
